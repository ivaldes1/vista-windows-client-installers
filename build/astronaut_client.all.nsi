...
; Copyright (c) 2013 Astronaut, LLC
...
; Check for valid IP. No don't because Shuttle uses a URL.
;!include ValidateIP.nsh
; HKLM (all users) vs HKCU (current user) defines
!define env_hkcu 'HKCU "Environment"'
!define env_hklm 'HKCU "SYSTEM\CurrentControlSet\Control\Session Manager\Environment"'
...
!endif
!ifndef HOST
    !define HOST "Enter VistA server IP or name."
!ENDIF
...
;  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\TMG-CPRS.lnk" "$INSTDIR\tmg-cprs\CPRSChart.exe" "S=%ASTRO_LOCAL_HOST% P=%ASTRO_PORT% CCOW=DISABLE SPOOF-VER=%ASTRO_CPRS_SPOOF%"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\TMG-CPRS.lnk" "$INSTDIR\tmg-cprs\CPRSChart.exe" "S=%ASTRO_SSH_HOST% P=%ASTRO_PORT% CCOW=DISABLE SPOOF-VER=%ASTRO_CPRS_SPOOF% SSHUSER=%ASTRO_SSH_clientID% plink sshpw=%ASTRO_SSH_CLIENT_PASS% sshport=%ASTRO_PORT%"
;  CreateShortCut "$DESKTOP\TMG-CPRS.lnk" "$INSTDIR\tmg-cprs\CPRSChart.exe" "S=%ASTRO_LOCAL_HOST% P=%ASTRO_PORT% CCOW=DISABLE SPOOF-VER=%ASTRO_CPRS_SPOOF%"
  CreateShortCut "$DESKTOP\TMG-CPRS.lnk" "$INSTDIR\tmg-cprs\CPRSChart.exe"  "S=%ASTRO_SSH_HOST% P=%ASTRO_PORT% CCOW=DISABLE SPOOF-VER=%ASTRO_CPRS_SPOOF% SSHUSER=%ASTRO_SSH_clientID% plink sshpw=%ASTRO_SSH_CLIENT_PASS% sshport=%ASTRO_PORT%"
...
; Shortcuts
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
  CreateDirectory "$SMPROGRAMS\$ICONS_GROUP"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\VistA Clinical Scheduling.lnk" "$INSTDIR\Clinical_Scheduling\ClinicalScheduling.exe" "/s=127.0.0.1 /p=9260"
  CreateShortCut "$DESKTOP\VistA Clinical Scheduling.lnk" "$INSTDIR\Clinical_Scheduling\ClinicalScheduling.exe" "/s=127.0.0.1 /p=9260"
  !insertmacro MUI_STARTMENU_WRITE_END
SectionEnd
...
;  CreateShortCut "$SMPROGRAMS\Startup\Astronaut SSH.lnk" "$INSTDIR\Putty\putty.exe" "-ssh -l %ASTRO_SSH_clientID% -pw %ASTRO_SSH_CLIENT_PASS% -L %ASTRO_PORT%:${VPN_HOST}:%ASTRO_PORT% %ASTRO_SSH_HOST%" "" "" SW_SHOWMINIMIZED
  Delete "$SMPROGRAMS\Startup\Astronaut SSH.lnk"
...
Section "Session Manager" SEC12
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\Sessions\Add or Edit....lnk" "$INSTDIR\Session_Manager\astro_instance_parms0.2.exe"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC04} "TMG-Config."
...
;       StrCpy $ICONS_GROUP "Astronaut"
	...
	${NSD_CreateLabel} 0 0 100% 12u "Astronaut VistA server IP Address or Name:"
	Pop $VISTA_HOST_LABEL

	${NSD_CreateText} 10u 13u 100% 12u "${HOST}"
	Pop $VISTA_HOST_CONTROL

	${NSD_CreateLabel} 0 30u 100% 12u "Astronaut VistA Port (use default if unsure):"
	Pop $VISTA_PORT_LABEL

	${NSD_CreateText} 10u 43u 100% 12u "${PORT}"
...
;	MessageBox MB_OK "You chose Port :$\n$\n$VISTA_PORT"
	${NSD_GetText} $VISTA_HOST_CONTROL $VISTA_HOST
        Push $VISTA_HOST
        ${If} $VISTA_HOST == "${HOST}"
  	  MessageBox MB_OK "Invalid IP: $VISTA_HOST $\n$\nEnter valid VistA server IP address or name."
  	  Abort
        ${EndIf}

;        Call ValidateIP
;        ${If} ${Errors}
;  	  MessageBox MB_OK "Invalid IP: $VISTA_HOST $\n$\nEnter valid VistA server IP address."
;  	  Abort
;        ${EndIf}
   ; set variable
   ; delete variable
   
   ; Delete system wide environment variables installed by previous
   ...
   DeleteRegValue ${env_hkcu} ASTRO_PROFILE
   WriteRegExpandStr ${env_hkcu} ASTRO_PROFILE "${DEFAULT_PROFILE}"
   DeleteRegValue ${env_hkcu} ASTRO_CPRS_SPOOF
   WriteRegExpandStr ${env_hkcu} ASTRO_CPRS_SPOOF "${CPRS_SPOOF}"
   DeleteRegValue ${env_hkcu} ASTRO_SSH_HOST
   WriteRegExpandStr ${env_hkcu} ASTRO_SSH_HOST "$VISTA_HOST"
   DeleteRegValue ${env_hkcu} ASTRO_LOCAL_HOST
   WriteRegExpandStr ${env_hkcu} ASTRO_LOCAL_HOST "${VPN_HOST}"
   DeleteRegValue ${env_hkcu} ASTRO_PORT
   WriteRegExpandStr ${env_hkcu} ASTRO_PORT "$VISTA_PORT"
   DeleteRegValue ${env_hkcu} ASTRO_textID
   WriteRegExpandStr ${env_hkcu} ASTRO_textID "text$VISTA_PORT"
   DeleteRegValue ${env_hkcu} ASTRO_SSH_clientID
   WriteRegExpandStr ${env_hkcu} ASTRO_SSH_clientID "client$VISTA_PORT"
   DeleteRegValue ${env_hkcu} ASTRO_TEXT_PASS
   WriteRegExpandStr ${env_hkcu} ASTRO_TEXT_PASS "${TEXT_CLIENT_PASS}"
   DeleteRegValue ${env_hkcu} ASTRO_SSH_CLIENT_PASS
   WriteRegExpandStr ${env_hkcu} ASTRO_SSH_CLIENT_PASS "${SSH_CLIENT_PASS}"
...
;  Delete "$SMPROGRAMS\$ICONS_GROUP\*"
  ...
  Delete "$DESKTOP\TMG-CPRS-1.0.lnk"
  Delete "$DESKTOP\TMG-CPRS-11.3.lnk"
  Delete "$DESKTOP\TMG-CPRS-11.4.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.7.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.8.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.9.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.10.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.11.lnk"
  Delete "$DESKTOP\TMG-CPRS-12.12.lnk"
  Delete "$DESKTOP\TMG-CPRS-13.03.lnk"
  Delete "$DESKTOP\TMG-CPRS.lnk"
